...
/**
 * Fetch CFTC Commitments of Traders data via Nasdaq Data Link (Quandl).
 * Returns Map<date, netNonCommercialPercent> — net non-commercial as % of open interest.
 */
async function fetchCFTCCOT(cftcTicker: string, startDate: string): Promise<Map<string, number>> {
  const result = new Map<string, number>();
  const apiKey = Deno.env.get('NASDAQ_DATA_LINK_KEY') || '';
  if (!apiKey || !cftcTicker) return result;
  try {
    const code = cftcTicker.replace('/', '/');
    const url = `https://data.nasdaq.com/api/v3/datasets/${code}.json?start_date=${startDate}&api_key=${apiKey}&order=asc`;
    const r = await fetchWithRetry(url, { headers: { 'User-Agent': UA } });
    if (!r.ok) { console.error(`CFTC ${cftcTicker}: HTTP ${r.status}`); return result; }
    const j = await r.json();
    const cols: string[] = (j.dataset?.column_names || []).map((c: string) => c.toLowerCase());
    const rows: any[][] = j.dataset?.data || [];
    // Find column indices for non-commercial long, short, and open interest
    ...
    // Fallback: try positions-based columns (FO_ALL format)
    ...
    if (longI < 0 || shortI < 0) { console.error(`CFTC: cannot find long/short columns in`, cols); return result; }
    for (const row of rows) {
      const date = row[0]; // YYYY-MM-DD
      const ncLong = parseFloat(row[longI]);
      const ncShort = parseFloat(row[shortI]);
      if (isNaN(ncLong) || isNaN(ncShort)) continue;
      const net = ncLong - ncShort;
      // Express as net long % of open interest (if available), else as raw net
      if (oiI >= 0) {
        const oi = parseFloat(row[oiI]);
        if (!isNaN(oi) && oi > 0) {
          result.set(date, +((net / oi) * 100).toFixed(2));
        }
      } else {
        // Normalize to 0-100 scale: map net contracts to a percentage using a heuristic
        result.set(date, +net.toFixed(0));
      }
    }
...
/**
 * Fetch historical market breadth (% > 200 DMA) from Financial Modeling Prep.
 * Returns Map<date, percentAbove200DMA>.
 */
async function fetchFMPBreadth(exchange: string, startDate: string): Promise<Map<string, number>> {
  ...
  try {
    const url = `https://financialmodelingprep.com/stable/market-breadth?type=${exchange}&apikey=${apiKey}`;
    const r = await fetchWithRetry(url, { headers: { 'User-Agent': UA } });
    if (!r.ok) { console.error(`FMP breadth ${exchange}: HTTP ${r.status}`); return result; }
    const j = await r.json();
    // FMP returns array of { date, averageAbove200DMA, ... } or similar
    if (Array.isArray(j)) {
      for (const row of j) {
        const date = row.date;
        const pct = row.averageAbove200DMA ?? row.percentAbove200DMA ?? row.breadth ?? null;
        if (date && pct != null && !isNaN(pct)) {
          // FMP may return as decimal 0-1 or percent 0-100
          const val = pct > 1 ? pct : pct * 100;
          result.set(date, +val.toFixed(2));
        }
      }
    }
 ...
 * CFTC COT positioning, FMP breadth, computes forward PE, dynamic P/B, ROE,
 * MCap/GDP, and upserts into daily_eyby_data.
 */
async function refreshUSIndex(
  sb: any, idx: string,
  indexConfig: typeof INDICES[string]
): Promise<{ merged: number; fetchedCloses: number; fetchedBY: number; fetchedWilshire: number; fetchedGDP: number; fetchedCFTC: number; fetchedBreadth: number; latestDate: string | null }> {
  ...
  if (closeSeries.length === 0) {
    return { merged: 0, fetchedCloses: 0, fetchedBY: 0, fetchedWilshire: 0, fetchedGDP: 0, fetchedCFTC: 0, fetchedBreadth: 0, latestDate: lastDate };
  }
  const tradingDates = closeSeries.map(c => c.date);

  // 2. FRED bond yields + GDP, Yahoo Wilshire 5000, CFTC COT, FMP Breadth — all in parallel
  ...
  const [rawBY, rawGDP, wilshireSeries, rawCFTC, rawBreadth] = await Promise.all([
    ...
    fetchFRED('GDP', fredKey, BASELINE_START_ISO),
    fetchYahooDailySeries('^W5000', fromStr, toStr),
    fetchCFTCCOT(indexConfig.cftcTicker || '', fromStr),
  ...
  const cftcFilled = forwardFill(tradingDates, rawCFTC);
  ...
  // Dynamic P/B from BVPS anchor
  const bvpsAnchor = parseFloat(Deno.env.get(indexConfig.bvpsEnvVar || '') || '0')
    || BVPS_DEFAULTS[idx] || 0;

  // 4. Build rows with MCap/GDP, CFTC COT, Breadth, dynamic P/B, ROE
  const rows: any[] = [];
  for (const { date, close } of closeSeries) {
    ...
    // Dynamic P/B = live price / BVPS anchor
    ...
    // CFTC COT and breadth
    const cftc = cftcFilled.get(date) ?? null;
    const breadth = breadthFilled.get(date) ?? null;
    rows.push({
      index_id: idx, date, pe, pb,
  ...
  return { merged, fetchedCloses: closeSeries.length, fetchedBY: rawBY.size,
    fetchedWilshire: rawWilshire.size, fetchedGDP: rawGDP.size,
    fetchedCFTC: rawCFTC.size, fetchedBreadth: rawBreadth.size,
    latestDate: latestInserted };
}
  ...
  try {
    if (mode === 'get-data') {
      ...
      const selectCols = idxConfig?.source === 'us'
        ? 'date, pe, pb, by_yield, close_price, mcap_gdp, cftc_net_pct, breadth_pct'
      ...
      if (idxConfig?.source === 'us' && allRows.length > 0) {
        ...
        const enriched = allRows.map((r: any, i: number) => {
          ...
          // ROE = PB / PE (Return on Equity %)
          const pb = r.pb || 0;
          const roe = (pb > 0 && pe > 0) ? +((pb / pe) * 100).toFixed(2) : null;
          // CFTC COT: normalize raw net % to 0-100 scale for the UI
          // Raw cftc_net_pct is net non-commercial as % of OI (can be negative)
          const rawCftc = r.cftc_net_pct;
          // Map to 0-100: 50 = neutral, >50 = net long, <50 = net short
          // Typical range is roughly -30% to +30%, map linearly
          const fiiNetLong = rawCftc != null ? Math.max(0, Math.min(100, 50 + rawCftc * 1.5)) : null;
          return {
            date: r.date, close, pe, pb,
            gsec10, fwd_ey,
            ...
            roe,
            fii_fut_net_long: fiiNetLong != null ? +fiiNetLong.toFixed(2) : null,
            breadth: r.breadth_pct != null ? +r.breadth_pct : null,
          };
        });
      ...
      // Indian indices: compact array format — now includes ROE as 6th element
      const compact = allRows.map((r: any) => {
        const roe = (r.pb > 0 && r.pe > 0) ? +((r.pb / r.pe) * 100).toFixed(2) : null;
        return [r.date, r.pe, r.pb, r.by_yield, r.close_price, roe];
      });
      ...
      if (indexConfig.source === 'us') {
        try {
          ...
          await logAudit(sb, idx, result.merged > 0 ? 'ok' : 'no-new-data', result.latestDate,
            `closes=${result.fetchedCloses} by=${result.fetchedBY} wil=${result.fetchedWilshire} gdp=${result.fetchedGDP} cftc=${result.fetchedCFTC} breadth=${result.fetchedBreadth} merged=${result.merged}`);
          return new Response(JSON.stringify({
            ...
            fetchedCFTC: result.fetchedCFTC, fetchedBreadth: result.fetchedBreadth,
