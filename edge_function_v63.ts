...
const DEPLOY_VERSION = 'v63-phase9.1';
...
function buildMacroCell(map: Map<string, number>, lagSteps: number, label: string, source: string, sourceUrl: string | null, opts: { yoyKind?: 'pct' | 'diff' | 'level' } = {}) {
  const yoyKind = opts.yoyKind || 'pct';
  if (!map || map.size === 0) return { value: null, date: null, yoy: null, status: 'unavailable', label, source, sourceUrl };
  ...
  return { value: latest != null ? +latest.toFixed(2) : null, date: latestDate, yoy, status: 'ok', label, source, sourceUrl };
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (mode === 'india-macro') {
      ...
      const [gdp, pfce, gfcf, iip, cpi, wpi, repo, gsec10, tbill3m, bankCredit, deposits, tradeBalance, fxReserves, brentSeries, niftySeries, niftyAutoSeries, systemRoe] = await Promise.all([
        ...
        fetchFRED('INDPROINDMISMEI',  fredKey, startISO),
        ...
        fetchFRED('INDMABMM301IDM',   fredKey, longStartISO),
        fetchFRED('MABMM301INM189S',  fredKey, longStartISO),
        fetchFRED('INDXTBAL',         fredKey, longStartISO),
        fetchFRED('TRESEGINM052N',    fredKey, longStartISO),
      ...
      const out = {
        ...
        industrial: {
          iip:     buildMacroCell(iip, 12, 'Index of Industrial Production (YoY)', 'FRED INDPROINDMISMEI (monthly)', FRED_URL('INDPROINDMISMEI')),
          coreInd: buildMacroCell(iip, 12, '8 Core Industries (IIP proxy YoY)',    'FRED INDPROINDMISMEI (proxy, monthly)', FRED_URL('INDPROINDMISMEI')),
        ...
        banking: {
          credit:   buildMacroCell(bankCredit, 12, 'Systemic Bank Credit (YoY)',     'FRED INDMABMM301IDM (RBI proxy, monthly)', FRED_URL('INDMABMM301IDM')),
          deposits: buildMacroCell(deposits,   12, 'Aggregate Deposit Growth (YoY)', 'FRED MABMM301INM189S (M3 proxy, monthly)', FRED_URL('MABMM301INM189S')),
        },
        external: {
          tradeBalance: buildMacroCell(tradeBalance, 12, 'Merchandise Trade Balance (USD)', 'FRED INDXTBAL (monthly)', FRED_URL('INDXTBAL')),
          fxReserves:   buildMacroCell(fxReserves,   12, 'FX Reserves ex Gold (USD)',        'FRED TRESEGINM052N (monthly)', FRED_URL('TRESEGINM052N')),
        ...
        businessCycle: {
          ...
          roeGsecSpread:  roeGsecSpreadVal != null ? { value: roeGsecSpreadVal, date: systemRoe.date, yoy: null, status: 'ok', label: 'ROE − 10Y G-Sec Spread (Capital-deployment edge)', source: `System ROE ${systemRoe.roe ?? '—'}% − G-Sec ${latestGSec != null ? latestGSec.toFixed(2) : '—'}%`, sourceUrl: FRED_URL('INDIRLTLT01STM') } : unavailableCell('ROE − G-Sec Spread', 'Awaiting ROE and bond-yield data', null),
        },
